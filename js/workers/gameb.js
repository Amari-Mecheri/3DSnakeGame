...
var score, highScore, snakeLength, round, snakePosition, snakeTail, direction;
var clearBoardView;
    ...
    for(let i=0; i<listSprite.length; i++){
      ...
      switch(sprite.Value){
        ...
        case candyBody:
          ...
          break;
        case snakePart:
          setSnakePart(sprite.Position.X,sprite.Position.Y, JSON.parse(SnakeDirection()));
          break;
...
//self.importScripts('workerConsts.js','./wasm/wasm_exec.js','./wasm/initWsm.js','./wasm/launchGame.js');
...
sendListSprite = (listSprite) => {
    if(listSprite){
        for(let i=0; i<listSprite.length; i++){
          ...
          Atomics.store(sharedArray, indexes.ListSprites + i*3 +1, sprite.Position.X);
          Atomics.store(sharedArray, indexes.ListSprites + i*3 +2, sprite.Position.Y);
        }
    }
}
